...
async def dont_be_like_this(ctx: discord.ApplicationContext, message: discord.Message):  
    try:
        webhook_url = GuildSettings.get(GuildSettings.ServerID == ctx.guild.id).WebHookURL
    except Exception as e:
        await ctx.send_response(f"You cannot use the bot without setting it up first! Run the /setup command to set the bot up!")
